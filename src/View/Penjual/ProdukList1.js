...
class ProdukList1 extends Component{
    ...
    var header = new Headers();
        ...
        fetch(`http://23.97.55.83/be1/product/View/${decoded.id_user}`,{
            method:"GET",
            header:header,
        })
        .then(res=> res.json())
    ...
    UpdateData=()=>{
        ...
        axios.put('http://23.97.55.83/be1/product/editproduk/'+this.state.form.id_produk,newProduk,config).then(response=>{
            this.ModalInsert();
            this.ShowData();
    ...
    DeleteData=()=>{
        axios.delete('http://23.97.55.83/be1/product/'+this.state.form.id_produk).then(response=>{
            this.setState({
                ModalDelete: false
            })
            this.ShowData();
